            ...
            <div class="navbar navbar-main navbar-expand-lg">
                <div class="container">
                    <a class="navbar-brand" href="{{url('/')}}">
                        <img alt="" title="" src="{{url('/')}}/asset/{{$logo->image_link}}" width="220">
                    </a>
                    ...
                    <div class="collapse navbar-collapse navbar-collapse-overlay" id="navbar-main-collapse">
                        <ul class="navbar-nav ml-auto">
                            ...
                            <li class="nav-item mm-in px-dropdown">
                                <a class="nav-link">{{__('Payment')}}</a>
                                <ul class="px-dropdown-menu mm-dorp-in">
                                    @if($set->transfer==1)      
                                    <li><a href="{{route('user.transfer')}}">{{__('Bill Payment')}}</a></li>
                                    @endif
                                    @if($set->request_money==1)
                                    <li><a href="{{route('user.request')}}">{{__('Transfer Money')}}</a></li>
                                    @endif
                                    @if($set->vcard==1)
                                    <li><a href="{{route('user.virtualcard')}}">{{__('Virtual Cards')}}</a></li>
                                    @endif
                                    @if($set->bill==1) 
                                    <li><a href="{{route('user.airtime')}}">{{__('Collect Payment')}}</a></li>
                                    @endif

                                    @if($set->bill==1) 
                                    <li><a href="{{route('user.airtime')}}">{{__('Website Integration')}}</a></li>
                                    @endif
                            ...
                            <li class="nav-item mm-in px-dropdown">
                                <a class="nav-link">{{__('Store')}}</a>
                                <ul class="px-dropdown-menu mm-dorp-in">
                                    @if($set->transfer==1)      
                                    <li><a href="{{route('user.transfer')}}">{{__('Storefront')}}</a></li>
                                    @endif
                                    @if($set->request_money==1)
                                    <li><a href="{{route('user.request')}}">{{__('Payment Link')}}</a></li>
                                    @endif
                                    @if($set->vcard==1)
                                    <li><a href="{{route('user.virtualcard')}}">{{__('Invoice')}}</a></li>
                                    @endif
        ...
        <div class="footer-top">
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-lg-6 m-15px-tb">
                        <h5 class="footer-title">{{__('Help')}}</h5>
                        <div class="row">
